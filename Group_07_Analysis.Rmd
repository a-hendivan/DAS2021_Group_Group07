...
\end{itemize}
The aim of this analysis is to find which of these properties if any, are significant in predicting whether a film will get IMBd rate over 7 or below 7. Thus  before we continue we create a new binary variable with takes value 1 if the rating is over 7 and 0 otherwise. 
We first run an exploratory analysis to detect any anomalies in the data set and to determine an appropriate class of model to fit. Next we proceed and fit the model and interpret the results of the fit.
Finally 

# Exploratory Data Analysis {#sec:EDA}

Taking a glance at the summaries of the variables shown in Table \ref{tab:summary} we notice that length variable has many outliers , and that is explained by the big difference between its maximum value and its third quartile value ,that is  399 minutes and 100 minutes respectively. 
Budget also has a wide range of variability and an amount of outliers . 
It is worth mentioning that votes have a large standard deviation , 4370 to be exact and that might be because of an outlier that presents a huge difference with the rest. Possibly a small number of films have comparatively huge number of votes compared to the rest of the films.
...
ggplot(film, aes(x=rating)) + 
   ...
   theme_light()
```
## Explore the explainatory variables in order

Firstly we explore the genre property.
...
ggplot(film , aes(x=genre)) + 
   ...
   theme_light()
```
Given the barplot in Figure\ref{fig:genre} we notice that the majority of films are within one of the 3 most dominant categories - Action, Comedy and Drama. On the opposite Animation Documentary and Short films have relatively lower frequencies. Finally the population of Romance films is very small compared to the rest films in our list. Only `r nrow(film[film$genre=="Romance",]) ` films are Romantic among the total of `r nrow(film) ` films.
...
have higher probabilities getting rate below 7 compared to the other genres , while Documentary and Short types are more likely to get rate higher that 7. 
Moving on to the next density plot of the rate for each genre we notice that some genres have very similar lines. In particular we see that documentary and short films have very similar distribution of rate. The same applies for animation and comedy. As for the remaining genres, romance ,action and drama we do see some more noticeable differences in their lines but the overall shape and characteristics are very similar. These observations might lead to regroup the genres to a new categorical variable of 3 levels instead of 7. 


Next we explore the budget property.   
...
Given the boxplot in Figure\ref{fig:budget} although we notice that the box of rate over 7 is slightly higher than the box of rate below 7 however there is a substantial overlap between the two.

Next we explore the length property.
...
Next we explore the year property.
...
Next we explore the votes property.
...
## Explore the relationship between explanatory variables

We might now check if there is any potential structure among these explanatory variables. 
Because we have identified that genre and length are the most important factors that influence the rating to be over 7, so first we check if genres and duration have any hidden structure. 
...
Recall we also noticed that length of the film might be a significant property that can determine the rating scale we proceed and remove observations that have missing values for length.The length and votes variables do not follow the bell curve, so we log transform this data to make it as “normal” as possible so that the statistical analysis results from it could become more valid.

The model we are going to fit to our data is the following.

$$ \log(\frac{p}{1-p}) = \beta_0 + \beta_1 \mathbb{I}_{A}+\beta_2 \mathbb{I}_{B} + \beta_3 \mathbb{I}_{C}   + \beta_4\cdot log(Length_i)+\epsilon_i, ~~~~ \epsilon_i \sim N(0, \sigma^2)$$
...
# We will apply model with genre, length as we assumed those are 
