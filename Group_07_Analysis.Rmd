...
It is worth mentioning that votes have a large standard deviation , 4370 to be exact and that might be because of an outlier that presents a huge difference with the rest. Possibly a small number of films have comparatively huge number of votings compared to the rest of the films.
As for the rating variable we do not observe any irregularities which is also confirmed by its density ploted in Figure \ref{fig:rating}.
...
Finally the population of Romance films is very small compared to the rest films in our list. Only `r nrow(film[film$genre=="Romance",]) ` films are Romantic among the total of `r nrow(film) ` films.
