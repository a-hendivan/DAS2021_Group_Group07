...
Given the contingency Table \ref{tab:cont}
 we can make four important notes. First, all Romance films have ratings below 7. 
Secondly, Animation and Short movies have a very little proportion that are rated below 7, only 3.3% and 0.1% respectively. 
Lastly Drama films are most liket to be rated below 7, with only 4% being rated over 7.
...
film2 <- film[!(film$genre %in% c("Romance","Drama","Animation","Short")),]
...
```{r modelfull, include=FALSE} 
# Full model with all variables 
model_full <- glm ( Rate ~ year + genre + length + log(votes) + budget , 
                    data= film2 , 
                    family = binomial(link = "logit"))
summary(model_full)
# in out put we see that year is not a significant explanatory , p-value = 0.4714 

mf_sum <- glance(model_full ) %>% 
   select(AIC,deviance) %>% 
   kable()
